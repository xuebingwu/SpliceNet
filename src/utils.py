...
def layer1_motif(weights,N,alpha,activation_func,output_label):
    ...
    # layer 1
    model2.add(Conv2D(n_motif, kernel_size=(4, l_motif), activation=activation_func, input_shape=rndseqs_shape, weights=weights))
    # get output on input sequences
...
def merge_models_with_job_name(
    ...
    for filename in os.listdir('.'):
        if fnmatch.fnmatch(filename,model_filename_pattern):
            i = i + 1
            print(TimeString(),'loading '+str(i)+' of '+str(n_model)+' models: '+filename+'                      ',end='\r')
            sys.stdout.flush()
            K.clear_session()
...
def merge_models(
        ...
        method,      # method to merge models: maxinfo, correlation, average
    ...
    if method == 'average':# average weighted by info content. doesn't work well
        ...
        for i in range(n_motif):
            ...
            merged_weights[2][(i*n_region):((i+1)*n_region)] =  merged_weights[2][(i*n_region):((i+1)*n_region)] * infos[0,i]   # position
...
def motif_regressor_net_one_region(l_seq,l_motif,use_bias_motif=True, use_bias_position=True):
    ...
    individual_motif_layer = Conv2D(1,kernel_size=(4,l_motif),activation='relu',input_shape = (4,l_seq,1),use_bias = use_bias_motif)
...
cor_train = calculate_exon_SF_correlation(x_train,y_train,1000)
