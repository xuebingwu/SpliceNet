...
def splice_net_model(
        ...
        use_constraints,        # kernal/bias contraints to enforce pwm-like weight matrix in convolutional filter layer: not working well
...
def splice_net_simulation(
        ...
        gamma_shape,
        gamma_scale,
        group_by,       # how training data is organized: EXPERIMENT, EXON (TODO), RANDOM #TODO: systematically compare options for motif and positionaleffect learning
    ...
    x_train,y_train, index, input_seqs_train = generate_training_data(seqs_train,[],expression_train,model,gamma_shape,gamma_scale,n_experiment_train,group_by,remove_non_regulated,0)
    x_test,y_test, index, input_seqs_test = generate_training_data(seqs_test,[],expression_train,model,gamma_shape,gamma_scale,n_experiment_test,group_by,remove_non_regulated,0)
...
def splice_net_training(
    ...
    for i in range(n_initialization):
        ...
        r1,r2,signloss,info,rnk = evaluate_trained_model(new_model,model0,new_model.predict(x_test2),y_test,positional_effect,motifs)
...
def evaluate_trained_model(
    ...
    signloss = sum(numpy.sign(positional_effect.flatten())==numpy.sign(model.layers[-1].get_weights()[0].flatten()))/float(positional_effect.shape[0])
...
def infinite_training(
    ...
    if model == []:
        ...
        model1 = splice_net_model(n_motif,n_region,l_seq,l_motif,output_activation,use_constraints,motif_combination,optimizer,l2_regularizer)
    
        # initialize the new model randomly, or with some parameters used in the simulation
        fraction_functional=1
        model,a,b = initialize_splice_net(model1,initialization_mode, model0,effect_scale,fraction_functional,n_mismatch)
        del model1
    ...
    while (loss_new <= loss_best or no_improvement < patience):
        ...
        r1,r2,signloss,info,rnk = evaluate_trained_model(model,model0,model.predict(x_test),y_test,positional_effect,motifs)
    ...
    r1,r2,signloss,info,rnk = evaluate_trained_model(best_model,model0,best_model.predict(x_test),y_test,positional_effect,motifs)
...
if __name__ == '__main__':
    ...
    parser.add_option("--load_simulator", dest = "simulator_job", help = "load simulator model and data from files",default = "")    
    ...
    parser.add_option("--n_motif_train", dest = "n_motif_train", help = "number of motif/RBP for training/prediction", type = int, default = -1)
    parser.add_option("--n_region", dest = "n_region", help = "number of regions surrounding each exon. The same RBP can have different effect on splicing when bind each region", type = int,default = 2)
    ...
    parser.add_option("--l2_regularizer", dest = "l2_regularizer", help = "parameter for l2 regularizer of the motif layer. Default 0 (or 0.1/n_motif?) ", type = float, default = 0)    
    parser.add_option("--effect_scale", dest = "effect_scale", help = "RBP positional effect scaling factor. Default 700 ", type = float, default = 700)  
    parser.add_option("--fraction_functional", dest = "fraction_functional", help = "Fraction of the regions that are functional. Default 1.0. ", type = float, default = 1.0)    
    # training
...
# simulation
    parser.add_option("--RBP_expr",dest = "RBP_expr",help = "RBP expression data, each row is an RBP and each column is an experiment (RNA-seq), if not specified, will use gamma distribution to simulate",default = "")
    parser.add_option("--shuffle_RBP_expr", dest = "shuffle_RBP_expr", help = "if true, will shuffle RBP expressions", default = False, action = 'store_true')
    parser.add_option("--gamma_shape", dest = "gamma_shape", help = "Gamma distribution shape parameter for RBP expression simulation", type = float, default = 4)
    parser.add_option("--gamma_scale", dest = "gamma_scale", help = "Gamma distribution scale parameter for RBP expression simulation. Default = 200/sqrt(n_motif)", type = float, default = 0.25)
    parser.add_option("--n_initialization", dest = "n_initialization", help = "number of initializations", type = int, default = 5)
    parser.add_option("--merge_method", dest = "merge_method", help = "how to merge models:maxinfo,correlation,average,vote", default = 'maxinfo')
    parser.add_option("--merge_power", dest = "merge_power", help = "power parameter used in merging models", type=float, default = 1.0)
    parser.add_option("--initialization_mode", dest = "initialization_mode", help = "how to initialize model during training: keras_default, copy_motif, copy_positional_effect, copy_both, simulation", default = "keras_default")
    parser.add_option("--group_by",dest = "group_by",help = "group training data by experiment, exon (TODO), or random",default = "experiment")
    parser.add_option("--remove_non_regulated", dest = "remove_non_regulated", help = "if true, will remove exons without any motif match (TODO)", default = False, action = 'store_true')
    parser.add_option("--no_plot", dest = "no_plot", help = "plot model diagram and output by default", default = False, action = 'store_true')
    parser.add_option("--no_motif_logo", dest = "no_motif_logo", help = "generate motif logos from filters in the convolutional layer. Need kpLogo installed", default = False, action = 'store_true')
    ...
    if options.simulator_job == "":
        ...
        model0,motifs,positional_effect = initialize_splice_net(model0,"simulation",[],options.effect_scale,1,options.n_mismatch)
    ...
    if model == []:
        print(TimeString(),"train a model, or a model initialized with motifs and / or positional effect from the simulation")
